...
- Keep environment-specific values in `environments/<name>/` and avoid hard-coding cluster names in shared manifests.
- Review `docs/TROUBLESHOOTING.md` if you are changing storage or Karpenter configuration.
...
3. Validate YAML and install behavior in a non-production or disposable cluster before merging.
...
- Include validation notes, especially when changing install scripts, Helm values, or Karpenter manifests.
...
If you are modifying install or cleanup flow, verify the script still matches the cluster assumptions in `scripts/install.sh` and `scripts/cleanup.sh`.
